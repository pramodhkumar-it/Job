<div class="row">
	...
	<div class="col-xs-12 col-sm-12 col-md-12">
		<div class="form-group">
			...
			@if ($selectedCategorys->count())

				@foreach($selectedCategorys as $selectedCategory)
					<option value="{{ $selectedCategory->id }}" {{ ($job->cat_id == $selectedCategory->id) ? 'selected="selected"' : '' }}>{{ $selectedCategory->name }}</option>    
				@endforeach
			@endif
